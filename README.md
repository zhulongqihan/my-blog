...
- **像素羊桌宠系统** - CSS box-shadow 像素画 + 多状态动画帧（行走/休息/进食）+ 拖拽交互 + 玩球/梳毛/召回/小憩等额外交互
- **AI 创意工坊** - 通义千问（DashScope OpenAI 兼容接口）+ SSE 流式输出，生成结构化前端方案（摘要/步骤/代码要点/验收清单），API Key 由 Nginx 运行时环境变量注入，前端零感知
- **AI Agent 项目入口** - 首页与文章页底部桥接模块、关于页与命令面板联动 AeroFlow Sentinel 在线演示入口，博客与独立 AI Agent 项目形成同一条内容线展示
...
- **博客联动方式**：已在首页/文章页底部桥接模块、关于页和命令面板中接入统一入口，并配套一组同主题文章，便于在简历和作品集中展示完整项目矩阵
...
### 2026-03-10（v2.1.2 AI 工坊修复 + 文章升级 + 像素羊增强）

- 修复 AI 创意工坊线上 502：Nginx 到 DashScope 的代理新增 SNI、IPv4 resolver 与运行时环境变量注入 Key，避免 TLS 握手失败和部署时覆盖线上密钥
- 首页、文章页、搜索页和归档链路统一改为优先使用 `publishedAt`，发布时间语义与排序保持一致
- 将 6 篇 AI / 工程化 / 生活向文章扩写为长文版本，补充封面图并打散发布时间，使内容线更自然
- 像素羊桌宠新增玩球、梳毛、召回、小憩等玩法，同时补充影子和面板信息，让交互更完整
- README 同步到当前部署方式：AI Agent 入口改为桥接式露出，AI 创意工坊 Key 改为 Compose 环境变量注入

### 2026-03-09（v2.1.1 AI Agent 项目联动 + 代码清理）

- 新增独立 AI Agent 项目入口：关于页、命令面板与后续桥接区统一接入 AeroFlow Sentinel 在线演示链接
